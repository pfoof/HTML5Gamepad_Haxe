...
class Gamepad extends FlxTypedGroup<FlxSprite>
{
	...
	function html5gamepadHandle()
	{
		var A = Main.HTML5_Gamepad.getButton(0);
		var B = Main.HTML5_Gamepad.getButton(1);
		var X = Main.HTML5_Gamepad.getButton(2);
		var Y = Main.HTML5_Gamepad.getButton(3);
		var START = Main.HTML5_Gamepad.getButton(8);
		var SELECT = Main.HTML5_Gamepad.getButton(9);

		var axisX = Main.HTML5_Gamepad.getAxis(0);
		var axisY = Main.HTML5_Gamepad.getAxis(1);

		updateStick(leftStick, axisX, axisY, LEFT_STICK_POS);
		updateButton(aButton, A);
		updateButton(bButton, B);
		updateButton(xButton, X);
		updateButton(yButton, Y);

		updateButton(startButton, START);
		updateButton(backButton, SELECT);
	}
